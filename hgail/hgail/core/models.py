...
class Network(object):
    ...
    def __init__(self, name, args=None): #TODO(yue)
        self.name = name
        self.reuse = False
        self.args=args
...
#TODO(debug) for control barrier function
class StateMLP(Network):
    def __init__(
            self,
            name,
            hidden_layer_dims,
            ...
            return_features=False,
            **kwargs):
        super(StateMLP, self).__init__(name=name, **kwargs)
        self.output_dim = output_dim
        self.obs_hidden_layer_dims = obs_hidden_layer_dims
        self.hidden_layer_dims = hidden_layer_dims
        self.activation_fn = activation_fn
    ...
    def _build(self, obs):
        ...
        with tf.variable_scope('{}/hidden'.format(self.name)):
            for hidden_dim in self.hidden_layer_dims:
                hidden = tf.contrib.layers.fully_connected(
                    hidden,
                    int(hidden_dim), #TODO(yue)
                    activation_fn=self.activation_fn,
                    biases_initializer=tf.constant_initializer(0.1),
                    weights_regularizer=tf.contrib.layers.l2_regularizer(self.l2_reg))
                features.append(hidden)
                if not self.args.remove_dropout:
                    hidden = tf.nn.dropout(hidden, self.dropout_keep_prob_ph)
    ...
    def forward(self, obs, deterministic=False):
        extra_feed = {self.dropout_keep_prob_ph: 1.} if deterministic else {}
        return self._run([obs], extra_feed=extra_feed)